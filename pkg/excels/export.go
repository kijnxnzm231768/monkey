...
func ExportExcel(list []interface{}, title string) (error, *excelize.File) {
	...
	/* -------------------- 第一行大标题 -------------------- */
	// 设置行高
	err := file.SetRowHeight(sheet1, 1, 25)
	if err != nil {
		return err, nil
	}
	// 合并单元格
	...
	err = file.SetCellValue(sheet1, "A1", title)
	if err != nil {
		return err, nil
	}
	...
	for k, v := range headers {
		err = file.SetCellValue(sheet1, string(65+k)+"2", v)
		if err != nil {
			return err, nil
		}
	}
	...
	for _, v := range list {
		...
		for i := 0; i < n; i++ {
			...
			err = file.SetCellValue(sheet1, string(65+num)+lineChr, getExp(name, expList, val.Interface()))
			if err != nil {
				return err, nil
			}
			num++
